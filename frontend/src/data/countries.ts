/**
 * ISO 3166-1 alpha-2 country list. Used by the mandatory Partner
 * `country` field (Settings → Partners, Profile). Codes match class-validator's
 * @IsISO31661Alpha2 on the backend.
 */
...
export const COUNTRIES: Country[] = [
  ...
  { code: 'BN', name: 'Brunei Darussalam' }, { code: 'BO', name: 'Bolivia' }, { code: 'BQ', name: 'Bonaire, Sint Eustatius and Saba' },
  ...
  { code: 'CD', name: 'Congo (DRC)' }, { code: 'CF', name: 'Central African Republic' }, { code: 'CG', name: 'Congo' },
  ...
  { code: 'KY', name: 'Cayman Islands' }, { code: 'KZ', name: 'Kazakhstan' }, { code: 'LA', name: 'Laos' },
  ...
  { code: 'MD', name: 'Moldova' }, { code: 'ME', name: 'Montenegro' }, { code: 'MF', name: 'Saint Martin' },
  ...
  { code: 'PN', name: 'Pitcairn' }, { code: 'PR', name: 'Puerto Rico' }, { code: 'PS', name: 'Palestine' },
  ...
  { code: 'SH', name: 'Saint Helena' }, { code: 'SI', name: 'Slovenia' }, { code: 'SJ', name: 'Svalbard and Jan Mayen' },
  ...
  { code: 'TN', name: 'Tunisia' }, { code: 'TO', name: 'Tonga' }, { code: 'TR', name: 'Türkiye' },
  { code: 'TT', name: 'Trinidad and Tobago' }, { code: 'TV', name: 'Tuvalu' }, { code: 'TW', name: 'Taiwan' },
  ...
  { code: 'UZ', name: 'Uzbekistan' }, { code: 'VA', name: 'Holy See (Vatican)' }, { code: 'VC', name: 'Saint Vincent and the Grenadines' },
  { code: 'VE', name: 'Venezuela' }, { code: 'VG', name: 'British Virgin Islands' }, { code: 'VI', name: 'U.S. Virgin Islands' },
...
const BY_CODE: Record<string, string> = Object.fromEntries(COUNTRIES.map((c) => [c.code, c.name]))

/** Human-readable name for an alpha-2 code (falls back to the code itself). */
export function countryName(code?: string | null): string {
  if (!code) return ''
  return BY_CODE[code.toUpperCase()] ?? code
}

/** Label like "Spain (ES)" for display. */
export function countryLabel(code?: string | null): string {
  if (!code) return '—'
  const n = BY_CODE[code.toUpperCase()]
  return n ? `${n} (${code.toUpperCase()})` : code
}
